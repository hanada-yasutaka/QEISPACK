...
!
!    Input, real ( kind = 16 ) T(N,3), contains the nonsymmetric matrix.  Its
...
!
!    Input/output, real ( kind = 16 ) E(N).  On input, E(2:N) contains the
...
!    transformed.
!
!    Input/output, real ( kind = 16 ) Z(N,M), contains the eigenvectors.
...
!
  ...
  do i = 2, n
    if ( e(i) == 0.0Q+00 ) then
      if ( t(i,1) /= 0.0Q+00 .or. t(i-1,3) /= 0.0Q+00 ) then
        ierr = 2 * n + i
        return
      end if
      e(i) = 1.0Q+00
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) A(N,N), the N by N matrix.  On output,
...
!
!    Output, real ( kind = 16 ) SCALE(N), contains information determining the
...
20 continue
  ...
  if ( j /= m ) then
    ...
    do i = 1, l
      call r16_swap ( a(i,j), a(i,m) )
    end do

    do i = k, n
      call r16_swap ( a(j,i), a(m,i) )
    end do
...
!
      if ( c /= 0.0Q+00 .and. r /= 0.0Q+00 ) then
...
!
!    Input, real ( kind = 16 ) SCALE(N), contains information determining
!    the permutations and scaling factors used by BALANC.
!
!    Input, integer ( kind = 4 ) M, the number of columns of Z to be
!    back-transformed.
!
!    Input/output, real ( kind = 16 ) Z(N,M), contains the real and imaginary 
...
!
!    Input/output, real ( kind = 16 ) A(N,MB).  On input, contains the lower
...
!
!    Output, real ( kind = 16 ) D(N), the diagonal elements of the tridiagonal
!    matrix.
!
!    Output, real ( kind = 16 ) E(N), the subdiagonal elements of the tridiagonal
!    matrix in E(2:N).  E(1) is set to zero.
!
!    Output, real ( kind = 16 ) E2(N), contains the squares of the corresponding
...
!
!    Output, real ( kind = 16 ) Z(N,N), the orthogonal transformation matrix
...
!    case, MBW=2*MB-1.
!
!    Input, real ( kind = 16 ) A(N,MBW), the lower triangle of the symmetric
...
!
!    Input, real ( kind = 16 ) E21, specifies the ordering of the eigenvalues
...
!
!    Input, real ( kind = 16 ) W(M), contains the M eigenvalues in ascending or
...
!
!    Input/output, real ( kind = 16 ) Z(N,M).  On input, the constant matrix
...
!
    if ( norm == 0.0Q+00 ) then
      norm = 1.0Q+00
    end if

    eps2 = 0.001Q+00 * norm * abs ( order)
      ...
      do i = 1, n
        ...
        if ( i /= n ) then
          ...
          if ( k /= i ) then
            ...
            do ij = i, jj, n
              call r16_swap ( rv(ij), rv(kj) )
              kj = kj + n
            end do

            if ( order == 0.0Q+00 ) then
              call r16_swap ( rv6(i), rv6(k) )
            end if
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) EPS1, is an absolute error tolerance for
...
!
!    Input, real ( kind = 16 ) D(N), the diagonal elements of the input matrix.
!
!    Input, real ( kind = 16 ) E(N), contains in E(2:N) the subdiagonal elements
!    of the matrix.  E(1) is arbitrary.
!
!    Input/output, real ( kind = 16 ) E2(N).  On input, the squares of the
...
!
!    Input, real ( kind = 16 ) LB, UB, define the interval to be searched for
...
!    in (LB,UB).
!
!    Output, real ( kind = 16 ) W(M), the eigenvalues in ascending order.
!
...
!    lower triangle of the matrix.
!
!    Input/output, real ( kind = 16 ) A(N,MB).  On input, A contains the lower
!    triangle of the symmetric band input matrix stored as an N by MB array.
!    Its lowest subdiagonal is stored in the last N+1-MB positions of the first
...
!
!    Input/output, real ( kind = 16 ) T.  On input, T specifies the shift (of
...
!
!    Input/output, real ( kind = 16 ) R.  On input for the first call, R should 
...
!
  ...
  real ( kind = 16 ) q
  real ( kind = 16 ) r
  real ( kind = 16 ) rv(2*mb*mb+4*mb-3)
  real ( kind = 16 ) s
  real ( kind = 16 ) scale
  real ( kind = 16 ) t
  real ( kind = 16 ) tst1
...
!
  if ( f <= 0.25Q+00 * r .or. 5 <= its ) then

    f = a(n,mb-1)

    if ( f /= 0.0Q+00 ) then
      q = ( a(n-1,mb) - g ) / ( 2.0Q+00 * f )
      s = pythag ( q, 1.0Q+00 )
      g = g - f / ( q + sign ( s, q ) )
    end if
...
!
!    Input, real ( kind = 16 ) SCALE(N), information determining the permutations
!    and scaling factors used by CBAL.
!
!    Input, integer ( kind = 4 ) M, the number of eigenvectors to be back
!    transformed.
!
!    Input/output, real ( kind = 16 ) ZR(N,M), ZI(N,M).  On input, the real
  ...
  do ii = 1, n
    ...
    if ( i < low .or. igh < i ) then
      ...
      if ( k /= i ) then
        ...
        do j = 1, m
          call r16_swap ( zr(i,j), zr(k,j) )
          call r16_swap ( zi(i,j), zi(k,j) )
        end do
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) AR(N,N), AI(N,N).  On input, the real and
...
!    I=IGH+1,...,N.
!
!    Output, real ( kind = 16 ) SCALE(N), information determining the
!    permutations and scaling factors used.
!
  implicit none
...
20 continue
  ...
  if ( j /= m ) then
    ...
    do i = 1, l
      call r16_swap ( ar(i,j), ar(i,m) )
      call r16_swap ( ai(i,j), ai(i,m) )
    end do

    do i = k, n
      call r16_swap ( ar(j,i), ar(m,i) )
      call r16_swap ( ai(j,i), ai(m,i) )
    end do
...
100 continue
  ...
  do jj = 1, l
     ...
     do i = 1, l
       if ( i /= j ) then
         if ( ar(j,i) /= 0.0Q+00 .or. ai(j,i) /= 0.0Q+00 ) then
           go to 120
         end if
       end if
     end do
...
140 continue
   ...
   do j = k, l
     ...
     do i = k, l
       if ( i /= j ) then
         if ( ar(i,j) /= 0.0Q+00 .or. ai(i,j) /= 0.0Q+00 ) then
           go to 170
         end if
       end if
     end do
...
!  Guard against zero C or R due to underflow.
!
     if ( c == 0.0Q+00 .or. r == 0.0Q+00 ) then
       go to 270
     end if
...
!  Parameters:
!
!    Input, real ( kind = 16 ) AR, AI, the real and imaginary parts of
!    the numerator.
!
!    Input, real ( kind = 16 ) BR, BI, the real and imaginary parts of
!    the denominator.
!
!    Output, real ( kind = 16 ) CR, CI, the real and imaginary parts of
!    the result.
!
  implicit none
...
!
!    Output, real ( kind = 16 ) WR(N), WI(N), the real and imaginary parts
!    of the eigenvalues.
!
...
!
!    Output, real ( kind = 16 ) ZR(N,N), ZI(N,N), the real and imaginary parts,
...
!    have been overwritten by other information.
!
!    Output, real ( kind = 16 ) W(N), the eigenvalues in ascending order.
!
!    Input, integer ( kind = 4 ) MATZ, is 0 if only eigenvalues are desired, and
!    nonzero if both eigenvalues and eigenvectors are to be computed.
!
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) AR(N,N), AI(N,N), the real and imaginary parts of
!    the complex Hessenberg matrix.
!
!    Input/output, real ( kind = 16 ) WR(N), WI(N).  On input, the real and
...
!
!    Output, real ( kind = 16 ) ZR(N,MM), ZI(N,MM), the real and imaginary parts
...
!
  ...
  real ( kind = 16 ) pythag
  real ( kind = 16 ) rlambd
  real ( kind = 16 ) rm1(n,n)
  real ( kind = 16 ) rm2(n,n)
  real ( kind = 16 ) rv1(n)
  real ( kind = 16 ) rv2(n)
  integer ( kind = 4 ) s
  logical select(n)
  ...
  real ( kind = 16 ) x
  real ( kind = 16 ) y
  real ( kind = 16 ) zi(n,mm)
  real ( kind = 16 ) zr(n,mm)
...
!
     do uk = k, n - 1
       ...
       if ( ar(uk+1,uk) == 0.0Q+00 .and. ai(uk+1,uk) == 0.0Q+00 ) then
         exit
       end if
...
!
     do i = 2, uk
        ...
        if ( pythag ( rm1(i,mp), rm2(i,mp) ) > &
          ...
          do j = mp, uk
            call r16_swap ( rm1(i,j), rm1(mp,j) )
            call r16_swap ( rm2(i,j), rm2(mp,j) )
          end do

        end if

        if ( rm1(mp,mp) == 0.0Q+00 .and. rm2(mp,mp) == 0.0Q+00 ) then
          rm1(mp,mp) = eps3
        end if

        call cdiv ( rm1(i,mp), rm2(i,mp), rm1(mp,mp), rm2(mp,mp), x, y )

        if ( x /= 0.0Q+00 .or. y /= 0.0Q+00 ) then
     ...
     if ( rm1(uk,uk) == 0.0Q+00 .and. rm2(uk,uk) == 0.0Q+00 ) then
       rm1(uk,uk) = eps3
     end if
...
!    of the matrix.
!
!    Input, real ( kind = 16 ) AR(N,IGH), AI(N,IGH), the multipliers which
...
!
  ...
  real ( kind = 16 ) ai(n,igh)
  real ( kind = 16 ) ar(n,igh)
  integer ( kind = 4 ) i
  ...
  do mm = low + 1, la
     ...
     do i = mp + 1, igh
        ...
        if ( xr /= 0.0Q+00 .or. xi /= 0.0Q+00 ) then
     ...
     if ( i /= mp ) then
       ...
       do j = 1, m
         call r16_swap ( zr(i,j), zr(mp,j) )
         call r16_swap ( zi(i,j), zi(mp,j) )
       end do
...
!
     if ( i /= m ) then
       ...
       do j = m - 1, n
         call r16_swap ( ar(i,j), ar(m,j) )
         call r16_swap ( ai(i,j), ai(m,j) )
       end do

       do j = 1, igh
         call r16_swap ( ar(j,i), ar(j,m) )
         call r16_swap ( ai(j,i), ai(j,m) )
       end do
    ...
    if ( xr /= 0.0Q+00 .or. xi /= 0.0Q+00 ) then
      ...
      do i = m + 1, igh
        ...
        if ( yr /= 0.0Q+00 .or. yi /= 0.0Q+00 ) then
...
!    routine CBAL.  If CBAL is not used, set LOW = 1 and IGH = N.
!
!    Input/output, real ( kind = 16 ) HR(N,N), HI(N,N).  On input, the real and
...
!    is to be performed.
!
!    Output, real ( kind = 16 ) WR(N), WI(N), the real and imaginary parts of the
...
300 continue
  ...
  if ( xr == 0.0Q+00 .and. xi == 0.0Q+00 ) then
    go to 340
  end if

  yr = ( hr(enm1,enm1) - sr) / 2.0Q+00
  yi = ( hi(enm1,enm1) - si) / 2.0Q+00
  call csroot ( yr**2-yi**2+xr, 2.0Q+00*yr*yi+xi, zzr, zzi )

  if ( yr * zzr + yi * zzi < 0.0Q+00 ) then
    zzr = -zzr
    zzi = -zzi
  end if
...
!
     do j = i - 1, en
       call r16_swap ( hr(i-1,j), hr(i,j) )
       call r16_swap ( hi(i-1,j), hi(i,j) )
     end do
...
!
    if ( 0.0Q+00 < wr(j) ) then

      do i = l, j
        call r16_swap ( hr(i,j-1), hr(i,j) )
        call r16_swap ( hi(i,j-1), hi(i,j) )
      end do
...
!    elements.
!
!    Input/output, real ( kind = 16 ) HR(N,N), HI(N,N).  On input, the real
...
!
     if ( 0.0Q+00 < wr(j) ) then
       ...
       do i = low, igh
         call r16_swap ( zr(i,j-1), zr(i,j) )
         call r16_swap ( zi(i,j-1), zi(i,j) )
       end do
...
!
  ...
  do nn = 2, n
     ...
     do ii = 1, enm1
        ...
        if ( yr == 0.0Q+00 .and. yi == 0.0Q+00 ) then
...
!    routine CBAL.  If CBAL is not used, set LOW = 1 and IGH = N.
!
!    Input/output, real ( kind = 16 ) ORTR(N), ORTI(N).  On input, information 
...
!    ORTR(J) and ORTI(J) to 0.0Q+00 for these elements.  On output, these arrays
...
!    correct for indices IERR+1,...,N.
!
!    Output, real ( kind = 16 ) ZR(N,N), ZI(N,N), the real and imaginary parts of
...
!
  ...
  integer ( kind = 4 ) nn
  real ( kind = 16 ) norm
  real ( kind = 16 ) orti(igh)
  real ( kind = 16 ) ortr(igh)
  real ( kind = 16 ) pythag
  real ( kind = 16 ) si
  real ( kind = 16 ) sr
  real ( kind = 16 ) ti
...
105 continue
  ...
  do ii = 1, iend
     ...
     if ( ortr(i) == 0.0Q+00 .and. orti(i) == 0.0Q+00 ) then
       go to 140
     end if

     if ( hr(i,i-1) == 0.0Q+00 .and. hi(i,i-1) == 0.0Q+00 ) then
       go to 140
     end if
!
...
!    of the matrix.
!
!    Input, real ( kind = 16 ) AR(N,IGH), AI(N,IGH), information about the 
!    unitary transformations used in the reduction by CORTH in their strict 
!    lower triangles.
!
!    Input/output, real ( kind = 16 ) ORTR(IGH), ORTI(IGH).  On input, further 
...
!    back transformed.
!
!    Input/output, real ( kind = 16 ) ZR(N,M), ZI(N,M).  On input, the real and 
...
!
  ...
  do mm = low + 1, la
    ...
    if ( ar(mp,mp-1) /= 0.0Q+00 .or. ai(mp,mp-1) /= 0.0Q+00 ) then
...
!
!    Output, real ( kind = 16 ) ORTR(IGH), ORTI(IGH), further information about 
!    the transformations.
!
  implicit none
...
!
    ...
    if ( f /= 0.0Q+00 ) then
      h = h + f * g
      g = g / f
      ortr(m) = ( 1.0Q+00 + g ) * ortr(m)
      orti(m) = ( 1.0Q+00 + g ) * orti(m)
    else
      ortr(m) = g
      ar(m,m-1) = scale
    end if
...
!  Parameters:
!
!    Input, real ( kind = 16 ) XR, XI, the real and imaginary parts of the 
!    quantity whose square root is desired.
!
!    Output, real ( kind = 16 ) YR, YI, the real and imaginary parts of the 
!    square root.
!
  implicit none
  ...
  tr = xr
  ti = xi
  s = sqrt ( 0.5Q+00 * ( pythag ( tr, ti ) + abs ( tr ) ) )
...
!
!    Input, real ( kind = 16 ) A(N,IGH), the multipliers which were used in the
...
!    transformed.
!
!    Input/output, real ( kind = 16 ) Z(N,M).  On input, the real and imaginary 
...
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, the matrix to be 
...
!
    if ( i /= m ) then
      ...
      do j = m - 1, n
        call r16_swap ( a(i,j), a(m,j) )
      end do

      do j = 1, igh
        call r16_swap ( a(j,i), a(j,m) )
      end do
...
!    interchanged in the reduction by ELMHES.
!
!    Output, real ( kind = 16 ) Z(N,N), the transformation matrix produced in the
!    reduction by ELMHES.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) T(N,3) contains the input matrix.  Its subdiagonal
...
!
!    Output, real ( kind = 16 ) D(N), the diagonal elements of the symmetric 
!    matrix.
!
!    Output, real ( kind = 16 ) E(N), contains the subdiagonal elements of
!    the symmetric matrix in E(2:N).  E(1) is not set.
!
!    Output, real ( kind = 16 ) E2(N), the squares of the corresponding elements 
...
!    matrix.
!
!    Output, real ( kind = 16 ) E(N), contains the subdiagonal elements of the 
!    symmetric matrix in E(2:N).  E(1) is not set.
!
!    Output, real ( kind = 16 ) Z(N,N), contains the transformation matrix
...
!
!    Input/output, real ( kind = 16 ) H(N,N), the N by N upper Hessenberg matrix.
...
!    IERR+1 through N.
!
!    Input/output, real ( kind = 16 ) Z(N,N).  On input, the transformation 
...
!
     if ( abs ( h(en,na) ) > abs ( h(na,en) ) ) then
       ...
       call cdiv ( 0.0Q+00, -h(na,en), h(na,na)-p, q, h(na,na), h(na,en) )
...
!
        ...
        if ( vr == 0.0Q+00 .and. vi == 0.0Q+00 ) then
...
!
!    Input, real ( kind = 16 ) A(N,N), contains information about the unitary
!    transformations used in the reduction by HTRID3.
!
!    Input, real ( kind = 16 ) TAU(2,N), contains further information about the
!    transformations.
!
!    Input, integer ( kind = 4 ) M, the number of eigenvectors to be back
!    transformed.
!
!    Input/output, real ( kind = 16 ) ZR(N,M), ZI(N,M).  On input, ZR contains 
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) AR(N,N), AI(N,N), contain information about
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, the lower triangle of 
...
!
!    Output, real ( kind = 16 ) D(N), the diagonal elements of the
!    tridiagonal matrix.
!
!    Output, real ( kind = 16 ) E(N), the subdiagonal elements of the tridiagonal
!    matrix in E(2:N).  E(1) is set to zero.
...
!    of E.  E2 may coincide with E if the squares are not needed.
!
!    Output, real ( kind = 16 ) TAU(2,N), contains further information about the
!    transformations.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) AR(N,N), AI(N,N).  On input, the real
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) D(N).  On input, the diagonal elements of
...
!
!    Input/output, real ( kind = 16 ) E(N).  On input, the subdiagonal elements
...
!    Input, real ( kind = 16 ) D(N), the diagonal elements of the input matrix.
!
!    Input, real ( kind = 16 ) E(N), the subdiagonal elements of the input matrix
...
!    submatrices.  E2(1) is also set to zero.
!
!    Output, real ( kind = 16 ) W(N), the eigenvalues in ascending order.  If an
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), the Hessenberg matrix.
!
!    Input/output, real ( kind = 16 ) WR(N), WI(N).  On input, the real and 
...
!    the eigenvectors.
!
!    Output, real ( kind = 16 ) Z(N,MM), the real and imaginary parts of the 
...
!
     do i = 2, uk
        ...
        if ( abs ( rm1(mp,i) ) > abs ( rm1(mp,mp) ) ) then
          ...
          do j = mp, uk
            call r16_swap ( rm1(j,i), rm1(j,mp) )
          end do

        end if

        if ( rm1(mp,mp) == 0.0Q+00 ) then
          rm1(mp,mp) = eps3
        end if
...
640    continue
     ...
     if ( rm1(uk,uk) == 0.0Q+00 .and. t == 0.0Q+00 ) then
       rm1(uk,uk) = eps3
     end if
!
...
!    of V.
!
!    Input/output, real ( kind = 16 ) A(NM,N). On input, the rectangular
...
!
!    Output, real ( kind = 16 ) W(N), the singular values of A.  These are the
...
!    be zero.
!
!    Input/output, real ( kind = 16 ) B(NM,IP).  On input, the constant column
...
!
!    Input, real ( kind = 16 ) A(N,IGH), contains information about the 
!    orthogonal transformations used in the reduction by ORTHES in its strict
!    lower triangle.
!
!    Input/output, real ( kind = 16 ) ORT(IGH), contains further information
...
!    transformed.
!
!    Input/output, real ( kind = 16 ) Z(N,N).  On input, the real and imaginary
...
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, the matrix.  On output,
...
!    Hessenberg matrix.
!
!    Output, real ( kind = 16 ) ORT(IGH), contains further information about the
!    transformations.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = 16 ) A, B, the two legs of a right triangle.
!
!    Output, real ( kind = 16 ) PYTHAG, the length of the hypotenuse.
!
  implicit none
...
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, the first real general
...
!
!    Input/output, real ( kind = 16 ) B(N,N).  On input, a real general matrix.
...
!
!    Output, real ( kind = 16 ) Z(N,N), contains the product of the right hand
...
!    Input, integer ( kind = 4 ) N, the order of the matrices.
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, a real upper Hessenberg 
...
!
!    Input/output, real ( kind = 16 ) B(N,N).  On input, a real upper triangular 
...
!
!    Input, real ( kind = 16 ) EPS1, a tolerance used to determine negligible
...
!    are to be accumulated for later use in computing eigenvectors.
!
!    Input/output, real ( kind = 16 ) Z(N,N).  If MATZ is FALSE, Z is not 
...
!    Input, integer ( kind = 4 ) N, the order of the matrices.
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, a real upper
...
!
!    Output, real ( kind = 16 ) ALFR(N), ALFI(N), the real and imaginary parts of
...
!
!    Output, real ( kind = 16 ) BETA(N), the diagonal elements of the 
...
!    to FALSE otherwise.
!
!    Input/output, real ( kind = 16 ) Z(N,N), is only used if MATZ is TRUE.
...
!    Input, integer ( kind = 4 ) N, the order of the matrices.
!
!    Input, real ( kind = 16 ) A(N,N), contains a real upper quasi-triangular 
...
!
!    Input, real ( kind = 16 ) ALFR(N), ALFI(N), BETA(N), vectors whose ratios
!      ( ALFR + I * ALFI ) / BETA
...
780     continue
        ...
        dr = w * zz - w1 * z1 - x * y
        di = w * z1 + w1 * zz - x1 * y
        if ( dr == 0.0Q+00 .and. di == 0.0Q+00 ) then
          dr = epsb
        end if
        go to 775
...
!
  ...
  do j = 1, n
    ...
    do i = 1, m
      ...
      r(i,j) = real ( seed, kind = 8 ) * 4.656612875Q-10
...
subroutine r16vec2_print ( n, a1, a2, title )
...
!    Input, integer ( kind = 4 ) N, the number of components of the vector.
!
!    Input, real ( kind = 16 ) A1(N), A2(N), the vectors to be printed.
!
!    Input, character ( len = * ) TITLE, a title.
!
  implicit none
  ...
  if ( all ( a1(1:n) == aint ( a1(1:n) ) ) .and. &
    ...
    do i = 1, n
      write ( *, * ) i, int ( a1(i) ), int ( a2(i) )
    end do
  else if ( all ( abs ( a1(1:n) ) < 1000000.0Q+00 ) .and. &
            all ( abs ( a2(1:n) ) < 1000000.0Q+00 ) ) then
    do i = 1, n
      write ( *, * ) i, a1(i), a2(i)
    end do
  else
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) EPS1.  On input, a theoretical absolute
...
!    in E(2:N).  E(1) is arbitrary.
!
!    Input/output, real ( kind = 16 ) E2(N).  On input, E2(2:N-1) contains the
...
!    a direct sum of submatrices.  E2(1) is set to 0.0Q+00 if the smallest
!    eigenvalues have been found, and to 2.0Q+00 if the largest eigenvalues
...
!
!    Output, real ( kind = 16 ) W(M), the M algebraically smallest eigenvalues in
...
!    those belonging to the second submatrix, and so on.
!
!    Output, real ( kind = 16 ) BD(N), contains refined bounds for the
...
!
  ...
  if ( jdef == 1 .and. tot < 0.0Q+00 ) then
    go to 140
  end if
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) B(N,N), contains information about the similarity
!    transformation (Cholesky decomposition) used in the reduction by REDUC
!    in its strict lower triangle.
!
!    Input, real ( kind = 16 ) DL(N), further information about the 
!    transformation.
!
!    Input, integer ( kind = 4 ) M, the number of eigenvectors to be back
!    transformed.
!
!    Input/output, real ( kind = 16 ) Z(N,M).  On input, the eigenvectors to be
...
!    minus sign.
!
!    Input/output, real ( kind = 16 ) A(N,N).  On input, A contains a real
...
!
!    Input/output, real ( kind = 16 ) B(N,N).  On input, the real symmetric
...
!
!    Input/output, real ( kind = 16 ) DL(N).  If N is negative, then the DL
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) A(N,N), the real general matrix.  On 
...
!    part first.
!
!    Output, real ( kind = 16 ) Z(N,N), contains the real and imaginary parts of 
...
!
!    Input/output, real ( kind = 16 ) A(N,N), B(N,N), the two real general 
!    matrices.  On output, A and B have been overwritten.
!
!    Input, integer ( kind = 4 ) MATZ, is zero if only eigenvalues are desired, 
!    and nonzero if both eigenvalues and eigenvectors are desired.
!
!    Output, real ( kind = 16 ) ALFR(N), ALFI(N), the real and imaginary parts,
!    respectively, of the numerators of the eigenvalues.
!
!    Output, real ( kind = 16 ) BETA(N), the denominators of the eigenvalues,
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), the real symmetric matrix.
!
!    Input, integer ( kind = 4 ) MATZ, is zero if only eigenvalues are desired, 
!    and nonzero if both eigenvalues and eigenvectors are desired.
!
!    Output, real ( kind = 16 ) W(N), the eigenvalues in ascending order.
!
!    Output, real ( kind = 16 ) Z(N,N), contains the eigenvectors, if MATZ
!    is nonzero.
!
...
!    of the matrix.
!
!    Input, real ( kind = 16 ) A(N,MB), contains the lower triangle of the real
...
!    Input, integer ( kind = 4 ) N, the order of the matrices A and B.
!
!    Input, real ( kind = 16 ) A(N,N), contains a real symmetric matrix.
!
!    Input, real ( kind = 16 ) B(N,N), contains a positive definite real
!    symmetric matrix.
!
!    Input, integer ( kind = 4 ) MATZ, is zero if only eigenvalues are desired, 
!    and nonzero if both eigenvalues and eigenvectors are desired.
...
!    Input, integer ( kind = 4 ) N, the order of the matrices A and B.
!
!    Input, real ( kind = 16 ) A(N,N), a real symmetric matrix.
!
!    Input, real ( kind = 16 ) B(N,N), a positive definite symmetric matrix.
!
!    Input, integer ( kind = 4 ) MATZ, is zero if only eigenvalues are desired, 
!    and nonzero if both eigenvalues and eigenvectors are desired.
!
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), the symmetric matrix.
!
...
!    Output, real ( kind = 16 ) W(N), the eigenvalues in ascending order.
!
!    Output, real ( kind = 16 ) Z(N,M), contains the orthonormal eigenvectors
...
!
  ...
  real ( kind = 16 ) a(n,n)
  real ( kind = 16 ) fwork1(n)
  real ( kind = 16 ) fwork2(n)
  real ( kind = 16 ) fwork3(n)
  real ( kind = 16 ) fwork4(n)
  integer ( kind = 4 ) ierr
...
!
!    Input, real ( kind = 16 ) A(NV), contains the lower triangle of the
...
!    Output, real ( kind = 16 ) W(N), the eigenvalues in ascending order.
!
!    Output, real ( kind = 16 ) Z(N,N), contains the eigenvectors, if MATZ is 
!    nonzero.
!
!    Output, integer ( kind = 4 ) IERR, is set to an error
!    completion code described in the documentation for TQLRAT and TQL2.
...
!
!    Input, real ( kind = 16 ) A((N*(N+1))/2), on input the lower triangle of the
!    real symmetric matrix, stored row-wise in the vector,
!    in the order A(1,1), / A(2,1), A(2,2), / A(3,1), A(3,2), A(3,3)/
!    and so on.
!
!    Output, real ( kind = 16 ) W(M), the eigenvalues requested.
!
...
!    and eigenvectors.
!
!    Output, real ( kind = 16 ) Z(N,M), the eigenvectors.
!
...
!
  ...
  logical type
  real ( kind = 16 ) w(m)
  real ( kind = 16 ) work1(n)
  real ( kind = 16 ) work2(n)
  real ( kind = 16 ) work3(n)
  real ( kind = 16 ) z(n,m)
!
!  IDEF =
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) W(N).  On input, the diagonal elements
!    of the real symmetric tridiagonal matrix.  On output, the eigenvalues in
!    ascending order.
!
!    Input, real ( kind = 16 ) E(N), the subdiagonal elements of the matrix in
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), contains the special real tridiagonal
...
!    the order of V.
!
!    Input, real ( kind = 16 ) A(M,N), the M by N matrix to be decomposed.
!
!    Output, real ( kind = 16 ) W(N), the singular values of A.  These are the
...
!
!    Output, real ( kind = 16 ) U(M,N), contains the matrix U, with orthogonal
...
!    decomposition is desired, and to FALSE otherwise.
!
!    Output, real ( kind = 16 ) V(N,N), the orthogonal matrix V of the
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) D(N), the diagonal elements of the matrix.
!
!    Input, real ( kind = 16 ) E(N), contains the subdiagonal elements of
!    the input matrix in E(2:N).  E(1) is arbitrary.
!
!    Input, real ( kind = 16 ) E2(N), contains the squares of the corresponding
...
!    and D(I-1).  E2(1) must contain 0.0Q+00 if the eigenvalues are in
!    ascending order, or 2.0Q+00 if the eigenvalues are in descending order.
...
!
!    Input, real ( kind = 16 ) W(M), the eigenvalues.
!
...
!    submatrix, and so on.
!
!    Output, real ( kind = 16 ) Z(N,M), the associated set of orthonormal
...
!    On output, E has been destroyed.
!
!    Input, real ( kind = 16 ) Z(N,N).  On input, the transformation matrix
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) D(N).  On input, D contains the diagonal
...
!
!    Input/output, real ( kind = 16 ) E2(N), contains in positions 2 through N 
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), contains information about the orthogonal
!    transformations used in the reduction by TRED1 in its strict lower
!    triangle.
!
!    Input, real ( kind = 16 ) E(N), the subdiagonal elements of the tridiagonal
...
!
!    Input, real ( kind = 16 ) A(NV), information about the orthogonal
...
!
!    Input/output, real ( kind = 16 ) A(N,N), on input, contains the real
...
!
!    Output, real ( kind = 16 ) D(N), contains the diagonal elements of the
!    tridiagonal matrix.
!
!    Output, real ( kind = 16 ) E(N), contains the subdiagonal elements of the
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real ( kind = 16 ) A(N,N), the real symmetric input matrix.  Only the
...
!    (N*(N+1))/2.
!
!    Input/output, real ( kind = 16 ) A(NV).  On input, the lower triangle of
...
!    matrix in E(2:N).  E(1) is set to zero.
!
!    Output, real ( kind = 16 ) E2(N),  the squares of the corresponding
!    elements of E.  E2 may coincide with E if the squares are not needed.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, real ( kind = 16 ) EPS1.  On input, an absolute error
...
!
!    Output, real ( kind = 16 ) LB, UB, define an interval containing exactly
!    the desired eigenvalues.
!
!    Output, real ( kind = 16 ) W(M), the eigenvalues between indices M11 and M22
!    in ascending order.
!
...
!    in (LB, UB).
!
!    Output, real ( kind = 16 ) W(M), the eigenvalues in ascending order if the
...
!
!    Output, real ( kind = 16 ) Z(N,MM), the associated set of orthonormal
